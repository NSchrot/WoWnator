<x-app-layout>
    <x-slot:title>
        WoWnator - Histórico de Palpites
    </x-slot:title>
    @php
    ...
    <x-slot name="header">
        <h2 class="font-heading font-bold text-2xl text-wow-gold leading-tight">
            {{ __('Histórico de Palpites') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-stone-800/70 shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-100 space-y-6">
                    @forelse($groupedGuesses as $date => $dailyGuesses)
                        <div x-data="{ open: {{ $loop->first ? 'true' : 'false' }} }" class="bg-stone-900/50 rounded-lg">
                            <div @click="open = !open" class="p-4 cursor-pointer flex justify-between items-center border-b border-stone-700">
                                <h3 class="text-xl font-bold text-wow-gold">
                    ...
                    @empty
                        <p class="text-center text-gray-400">Você ainda não fez nenhum palpite.</p>
                    @endforelse
                </div>
            </div>
        </div>
