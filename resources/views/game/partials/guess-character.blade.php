<div class="p-4 mt-2 border-t-0 border-l-0 border-r-0 border-b-0 rounded-lg">
        <div class="mb-8 animate-fade-in">
            <div class="bg-stone-900/50 p-6 rounded-lg shadow-lg mb-6">
                <h3 class="text-xl font-bold text-center text-wow-gold font-heading">Adivinhe o Personagem</h3>
            </div>
        
            @if(!$hasGuessedCorrectly['character'])
                <div class="text-center text-stone-400 mb-4">
                    Tentativas restantes: 
                    <span class="font-bold text-wow-gold">{{ 15 - $guesses['character']->count() }}</span>
                </div>
                ...
                <div x-data="{
                    ...
                    <form x-on:submit.prevent="if(selectedId) $el.submit()" action="{{ route('game.guess.character') }}" method="POST" class="flex items-center gap-2">
                        @csrf
                        <input type="hidden" name="tab" value="character">
            ...
            @else
                <div class="text-center p-4 bg-green-900/50 border border-green-700 rounded-lg max-w-md mx-auto animate-fade-in">
                    <p class="text-lg text-gray-200">
                        Você acertou na 
                        <span class="font-bold text-wow-gold">{{ $guesses['character']->count() }}ª</span> tentativa!
                    </p>
