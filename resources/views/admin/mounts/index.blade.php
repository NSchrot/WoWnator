    ...
    <div x-data="{ deleteModalOpen: false, deleteUrl: '', show: false }" x-init="$nextTick(() => show = true)">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-stone-700/50 border-t border-stone-700">
                        <td class="px-6 py-4 flex items-center gap-3">
                            <img src="{{ $mount->icon_url }}" class="h-10 w-10 rounded-md object-cover">
                            {{ $mount->name }}
                        </td>
                        <td class="px-6 py-4">{{ $mount->type }}</td>
                        <td class="px-6 py-4">{{ $mount->faction }}</td>
                        <td class="px-6 py-4">{{ $mount->source }}</td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end items-center gap-2 text-sm">
                                <a href="{{ route('admin.mounts.show', $mount) }}" class="px-3 py-1 border border-green-500 text-green-400 rounded-md hover:bg-green-500 hover:text-stone-900 transition font-semibold">Ver</a>
                                <a href="{{ route('admin.mounts.edit', $mount) }}" class="px-3 py-1 border border-blue-500 text-blue-400 rounded-md hover:bg-blue-500 hover:text-stone-900 transition font-semibold">Editar</a>
                                <button @click.prevent="deleteUrl = '{{ route('admin.mounts.destroy', $mount) }}'; deleteModalOpen = true" class="px-3 py-1 border border-red-500 text-red-500 rounded-md hover:bg-red-500 hover:text-white transition font-semibold">Apagar</button>
                            </div>
                        </td>
                    </tr>
                    @endforeach
        ...
        <div x-show="deleteModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.away="deleteModalOpen = false" x-cloak>
            <div class="bg-stone-800 rounded-lg p-6 w-full max-w-md">
                ...
                <p class="text-gray-400 mt-2">Tem a certeza de que deseja apagar esta montaria?</p>
